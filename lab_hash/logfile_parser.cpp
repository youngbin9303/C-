...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
		...
		if(!pageVisitedTable.keyExists(unique_URLs))
			uniqueURLs.push_back(unique_URLs);	
		pageVisitedTable[unique_URLs] = true;
...
//    (void) url;      // prevent warnings... When you implement this function, remove this line.
    ...
    string uniqueURLs = url;
    string key = customer.substr(1, customer.length()-3);
    key.append(uniqueURLs);
