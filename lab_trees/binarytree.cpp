...
bool BinaryTree<T>::isOrdered(const Node* subRoot) const
{
	...
	if(subRoot -> right != NULL){
		...
		if(root -> elem > subRoot -> right -> elem){
			return false;
		}
	}
	if((subRoot -> elem < subRoot -> right -> elem) || (subRoot -> elem > subRoot -> left -> elem)){
		return true;
	}
...
template <typename T>
void BinaryTree<T>::printPaths(const Node * subRoot, vector<T> paths) const{
	if(subRoot == NULL)
		return;
...
template <typename T>
int BinaryTree<T>::sumDistances(const Node * subRoot, int length) const
{
	int distance;
	if(subRoot == NULL)
		return 0;
	distance = length + sumDistances(subRoot->left, length+1) + sumDistances(subRoot->right, length+1);
	return distance;
}
