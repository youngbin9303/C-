...
class KDTree
{
  ...
  private:
   ...
   void quickSelect(int left, int right, int mid, int dimension);
   int partition(int left, int right, int pivotIndex, int dimension);
    
   void findNearestNeighbor_helper(const Point<Dim>& query, Point<Dim>& currentBest, int left, int right, int min_distance, int dimension, bool& first_trial) const;
